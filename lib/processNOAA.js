			...
			wxtoimg.on('exit', () => {
				...
				wxtoimg_map.on('exit', () => {
					...
					wxtoimg_no.on('exit', () => {
						...
						wxtoimg_msa.on('exit', () => {
							// remove this if it works as intended
							// -----------------------------------
							fs.copyFileSync(path + name + '_msa.png', path + name + '_msa.bad.png', (err) => {
								if (err) throw err;
								console.log('backed up "bad" MSA enhanced image - please verify exclusion');
							});
							// -----------------------------------
