	...
	config.satellites.forEach((sat) => {
		...
		if (index === -1) {
			...
			rawPasses.forEach((pass) => {
				...
				if (!sat.noaa) {
					console.log('METEOR Satellite! - Checking solar elevation...');
					let avgEclipseDepth = (startPos.eclipseDepth + endPos.eclipseDepth) / 2;
					console.log('Average Eclipse Depth (Should be < -25): ' + avgEclipseDepth.toString());
					if (avgEclipseDepth < -25) {
						allPasses.push({
						...
						console.log('Sun too low, skipping pass...')
					};
				} else {
					allPasses.push({
						start: Math.round(pass.start),
		...
		if (array[index + 1].start - item.end < 65000) {
			...
			if (array[index + 1].start < item.start + (item.duration / 2)) {
				...
				if ((item.sat.priority || array[index + 1].sat.priority) && (item.sat.priority !== array[index + 1].sat.priority)) {
					...
					// if neither have a priority, remove based max on elevation
				...
				allPasses[index].endDate = new Date(item.end);
