...
module.exports = () => {
	return new Promise(async (resolve, reject) => {
		...
		await commandExists('medet').catch(() => {
			console.error('ERROR: medet not installed!');
			failed = true;
		});
		await commandExists('rectify-jpg').catch(() => {
			console.error('ERROR: rectify-jpg not installed!');
			failed = true;
		});
		await commandExists('convert').catch(() => {
			console.error('ERROR: convert not installed!');
			failed = true;
		});
