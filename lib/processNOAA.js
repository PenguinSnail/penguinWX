		...
		wxmap.on('exit', () => {
			...
			wxtoimg.on('exit', () => {
				...
				wxtoimg_map.on('exit', () => {
					...
					wxtoimg_msa.on('exit', () => {
						if (pass.sat.email) {
							email(config, name, name, [path + name + '.png', path + name + '_map.png', path + name + '_msa_map.png']);
						};
						if (pass.sat.tweet) {
							tweet(config, pass, [path + name + '.png', path + name + '_map.png', path + name + '_msa_map.png']);
						};
					});
				});
			});
